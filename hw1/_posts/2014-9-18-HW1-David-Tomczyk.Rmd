...
title.size <- theme(plot.title = element_text(size = 30))
...
#Stacked bar chart
ggplot(progtwit, aes(Program, fill=Twitter))+ geom_bar()+ legend.order + colors.custom + twit.title + title.size + x.axis + y.axis + twit.legend #now title works?

#Dot plot
dotplot <- geom_dotplot(stackgroups=TRUE, binpositions="all")
ggplot(progtwit, aes(Program, fill=Twitter))+ dotplot + legend.order + colors.custom + twit.title + x.axis + y.axis+twit.legend+title.size
